...
func run() int {
	...
	if len(os.Args) < 3 {
		fmt.Fprintf(os.Stderr, "Usage: %s [knn | nn | nn2] [ iris | mnist ]", os.Args[0])
		return 1
	}
	...
	if cmd == "knn" && data == "iris" {
		...
		err = nnMNIST()
	} else if cmd == "nn2" && data == "iris" {
		err = nn2IRIS()
	} else if cmd == "nn2" && data == "mnist" {
		err = nn2MNIST()
	} else {
...
func nn2IRIS() (err error) {
	...
	err = evalNN2(trainData, testData)
	return
}
...
func nnMNIST() (err error) {
	var rawData base.FixedDataGrid
	t1 := time.Now()
	rawData, err = mnistdata.Load(mnistImageFile, mnistLabelFile, 100000)
	t2 := time.Now()
	fmt.Println("elapse time of loading :", t2.Sub(t1))
	if err != nil {
		return
...
func evalNN(trainData, testData base.FixedDataGrid, resolveClassName func([]float64) string, convData func(base.FixedDataGrid) [][][]float64) (err error) {
	...
	cls := nn.NewClassifier(100, 10000, 0.4, 0.6, true, resolveClassName, convData)
...
// evalNN2 : NN による分析
func evalNN2(trainData, testData base.FixedDataGrid) (err error) {
	cols, rows := trainData.Size()
	fmt.Printf("trainData = %d x %d\n", rows, cols)

	// MLNを初期化
	cls := neural.NewMultiLayerNet([]int{cols - 1, 100, 1})
